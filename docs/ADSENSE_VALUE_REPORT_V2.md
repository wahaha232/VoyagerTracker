...
| Performance | PageSpeed Insights (Google, Lighthouse 13.5, mobile, 2026-09-25): home 100 / 100 / 100 / 100 (FCP 1.5 s, LCP 1.5 s, TBT 30 ms, CLS 0); Compare 98 / 100 / 100 / 100 (FCP 1.8 s, LCP 2.0 s, TBT 0 ms, CLS 0). Order: Performance / Accessibility / Best Practices / SEO. Above-the-fold content is not faded in and the hero numbers keep their space before they are calculated. Remaining PSI hint: longer cache lifetimes, which GitHub Pages does not let a site configure |
...
| Item | Status | Note |
| --- | --- | --- |
| AdSense code / consent banner | Code on, consent message pending | `ADSENSE_CLIENT` in `scripts/pages.mjs` is set to the owner’s `ca-pub-1512317781873771`; the loader is on all 19 pages and the Privacy page describes ads as active (recorded on the Updates page). The owner still needs to publish Google’s consent message (AdSense → Privacy & messaging) for EEA/UK/Swiss visitors |
| `ads.txt` | Done | Served at https://wahaha232.github.io/ads.txt by the owner’s user-site repository (covers the whole domain, including /VoyagerTracker/); one line for the owner’s publisher ID, text/plain, HTTP 200 |
| Open Graph share image | Done | `public/og-image.png` (1200×630) generated by `npm run og` from the model; labelled as calculated estimates with its date |
| Instrument status and events | Manual | Must be updated by hand when NASA publishes news |
| Lighthouse performance | Measured | Local Lighthouse runs on the development PC vary widely (TBT ≈200–1,100 ms); PageSpeed Insights on Google’s servers is the reference figure above. Real-visitor (field) data is not available yet |
