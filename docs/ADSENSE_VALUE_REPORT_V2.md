...
| Performance | Lighthouse 12, mobile, simulated throttling (local preview): Performance 63–80, Accessibility / Best Practices / SEO 100, CLS 0 on home, tools, compare, voyager-1 and FAQ. Prerendered HTML is hydrated after the first paint; CSS inlined; monospace font self-hosted (no Google Fonts request); 3D model only auto-loads on wide screens; live counters pause off-screen and in hidden tabs. Ephemeris chunk 89 KB (34 KB gzip) |
...
| Lighthouse performance | Partly limited | Simulated mobile FCP is still ≈3.3–3.7 s, mainly from HTML + JavaScript size under the simulated slow network; real-device timings will differ |
