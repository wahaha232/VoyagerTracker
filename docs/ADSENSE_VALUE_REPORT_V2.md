...
| Performance | Lighthouse 12, mobile, simulated throttling, run locally against the live site: home 65, FAQ 84, compare 69; FCP/LCP 2.8–2.9 s; CLS 0; Accessibility / Best Practices / SEO 100. The local preview gave Performance 56–80 across home, tools, compare, voyager-1 and FAQ. Above-the-fold content is not faded in, and the hero numbers keep their space before they are calculated. Prerendered HTML is hydrated after the first paint; CSS inlined; monospace font self-hosted (no Google Fonts request); 3D model only auto-loads on wide screens; live counters pause off-screen and in hidden tabs. Ephemeris chunk 89 KB (34 KB gzip) |
...
| Item | Status | Note |
| --- | --- | --- |
| AdSense code / consent banner | Ready, switched off | Set `ADSENSE_CLIENT` in `scripts/pages.mjs` to the owner's own `ca-pub-…` ID (the old commit `aaae1a6` contains it) and the loader script and the Privacy page's ad section switch on together. Visitors from the EEA/UK/Switzerland need a Google-certified consent tool first |
...
| Lighthouse performance | Partly limited | Total Blocking Time varies strongly between runs on the test machine (≈200–1,100 ms on the home page), mostly from hydration and the first layout. PageSpeed Insights could not be run (keyless daily quota exhausted); real-device timings will differ |
