/**
 * events — dated, sourced Voyager mission events (single source of truth).
 *
 * Used by the timeline, the Updates log and the Date Explorer. Every entry
 * is written for this site from the linked NASA/JPL material; add an event
 * only when there is a published source for it.
 */

type Tri = { en: string; zh: string; es: string };
const T = (en: string, zh: string, es: string): Tri => ({ en, zh, es });
export type EventCraft = 'voyager1' | 'voyager2' | 'both';
